...
public class 스도쿠완성 {
    public static void main(String[] args) {
        ...
        int 배열크기[][] = new int[n][n];  //2차원 배열 n 크기로 생성
        ...
        int rand = random.nextInt(n) + 1; //랜덤상수 n만큼 자연수가 들어가게 함
        int 세로줄;
        int 가로줄;
        boolean 동일 = true;
        int 루프횟수;
            ...
            for (int i = 0; i < n; i++){
                for (int j = 0; j < n; j++) {
                    ...
                    while (true){
                        ...
                        for (int a = 세로줄 - 1; a >= 0; a--) {  //기준 왼쪽값들 일치 유뮤 검사
                            while (동일 == true) {
                        ...
                        for (int b = 가로줄 - 1; b >= 0; b--) {  //기준 위쪽값들 일치유무검사
                            while (동일 == true) {
