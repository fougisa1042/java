...
public class 별연습 {
    public static void main(String[] args) {
        ...
        b= n*2;
            ...
            //삼각상
            for (i = 1; i <= n; i++){  //n값을 5를 받는다고 친다 그리고 i , j , k,의 초기값은 전부 1 그리고 수행한 뒤에는 변수 값에다가 ++를 해준다
                for (j = 0; j <= a - i; j++){ //n - i = 5 - 1 = 4 이기 때문에
...
//별 왼쪽 다리
/*
for (i = a; i >= 1; i--) {  //i = n i >= 1 i--이기 떄문에 조건식이 충족하면 i값이 점점 적어진다
        for (j = a; j >= n - i; j--) { //처음에 n - i = 0 fals가 떠서 그냥 넘어 간다
        System.out.print(" ");   //
        }
        for (k = 1; k <= 2 * i - 1; k++) {  //
        System.out.print("*");  //
